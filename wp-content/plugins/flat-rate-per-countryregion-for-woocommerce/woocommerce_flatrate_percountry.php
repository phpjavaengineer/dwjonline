		...
		class WC_Flat_Rate_Per_Country_Region extends WC_Shipping_Method {
			...
			function init() {
				...
				if ( WC()->countries ) {
					$this->regions = array(
						...
						'EU_EUVAT' => array(
							'name' => __('European Union', 'flat-rate-per-countryregion-for-woocommerce').' + '.__('Monaco, Isle of Man and Great Britain', 'flat-rate-per-countryregion-for-woocommerce').' (EU VAT)',
							'countries' => array_merge( WC()->countries->get_european_union_countries(), array( 'MC', 'IM', 'GB' ) )
						),
						//Oceania
						'OC_ANZ' => array(
			...
			function wpml_shipping_method_label($label, $method) {
				if ( $method->id == $this->id ) {
					...
					if ( $method->cost > 0 ) {
						$tax_price_display_mode = version_compare( WC_VERSION, '4.4', '>=' ) ? WC()->cart->get_tax_price_display_mode() : WC()->cart->tax_display_cart;
						if ( $tax_price_display_mode == 'excl' ) {
